...
> Construye e integra herramientas personalizadas para extender la funcionalidad del Claude Agent SDK
...
<CodeGroup>
  ...
  // Crear un servidor MCP SDK con herramientas personalizadas
  const customServer = createSdkMcpServer({
    ...
    tools: [
      tool(
        ...
        {
          location: z.string().describe("Nombre de la ciudad o coordenadas"),
  ...
  @tool("get_weather", "Obtener el clima actual para una ubicación", {"location": str, "units": str})
  async def get_weather(args: dict[str, Any]) -> dict[str, Any]:
      ...
      return {
          "content": [{
              "type": "text",
              "text": f"Temperatura: {data['temp']}°\nCondiciones: {data['conditions']}\nHumedad: {data['humidity']}%"
          }]
      }

  # Crear un servidor MCP SDK con la herramienta personalizada
...
<CodeGroup>
  ...
  async function* generateMessages() {
    yield {
      ...
      message: {
        role: "user" as const,
        content: "Calcula 5 + 3 y traduce 'hola' al inglés"
      }
    };
  }
...
<CodeGroup>
  ...
  tool(
    ...
    {
      // El esquema Zod define tanto validación en tiempo de ejecución como tipos TypeScript
      data: z.object({
        name: z.string(),
  ...
  @tool(
      ...
      "Procesar datos estructurados con seguridad de tipos",
      {
          ...
          "age": int,
          "email": str,
          "preferences": list  # Los parámetros opcionales pueden manejarse en la función
      }
  )
  ...
  # Para esquemas más complejos, puedes usar formato JSON Schema
  @tool(
      "advanced_process",
...
* [Configuración del SDK](/es/api/agent-sdk/sdk-configuration)
